...
// A NULL ring_buffer_t pointer should be rejected before anything is touched.
...
// non-power-of-two values should all be rejected.
...
// Happy path: valid arguments succeed and the resulting buffer is empty.
...
// A NULL ring_buffer_t pointer should be rejected.
...
// A NULL element pointer should be rejected.
...
// A successful write should copy the element into the slot at the current head.
...
// Each successful write should advance head by one slot.
...
// With overwrite=true, writing to a full buffer should evict the oldest
// element (advancing tail) and succeed, leaving the remaining elements in
// the correct order for a subsequent read.
...
// overwrite=true on a non-full buffer should behave like a normal write -
// guards against the eviction branch firing regardless of fullness.
...
// A NULL destination pointer should be rejected.
TEST(RingBuffer, ReadRejectsNullElement)
{
    LONGS_EQUAL(STATUS_ERR_INVALID_ARG, ring_buffer_read(&rb, NULL));
}

// Reading from an empty buffer should fail rather than return stale data.
...
// Read should return the oldest (first-written) element, proving it reads
// from tail, not head.
...
// Each successful read should advance tail by one slot.
...
// Flush should empty a non-empty buffer and leave it usable for new writes
...
// A NULL pointer should be treated as "not empty" (a safe default), not crash.
TEST(RingBuffer, IsEmptyFalseOnNull)
{
    CHECK_FALSE(ring_buffer_is_empty(NULL));
}

// The buffer should report empty again once every written element has been read.
...
// A NULL pointer should be treated as "not full" (a safe default), not crash.
TEST(RingBuffer, IsFullFalseOnNull)
{
    CHECK_FALSE(ring_buffer_is_full(NULL));
}

// Reading one element out of a full buffer should free a slot, so is_full
// should flip back to false.
