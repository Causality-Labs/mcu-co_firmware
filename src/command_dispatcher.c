...
status_t dispatch_command(frame_t *frame, response_t *resp)
{
    ...
    case GPIO_CFG:
    {
        status_t ret = gpio_controller_cfg(frame->payload, frame->length);
        if (ret != STATUS_OK)
        {
            LOG_ERROR(MODULE_NAME, "gpio_controller_cfg() returned an error.");
            return ret;
        }
    ...
    case GPIO_IRQ:
    {
        status_t ret = gpio_controller_irq(frame->payload, frame->length);
        if (ret != STATUS_OK)
        {
            LOG_ERROR(MODULE_NAME, "gpio_controller_irq() returned an error.");
            return ret;
        }
